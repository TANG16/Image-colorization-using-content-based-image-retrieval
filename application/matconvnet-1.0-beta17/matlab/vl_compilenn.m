...
%
%   * Windows 7 x32, MATLAB R2014a, Visual C++ 2010 and CUDA Toolkit
%     6.5 (unable to compile with Visual C++ 2013).
%   * Windows 8 x32, MATLAB R2014a, Visual C++ 2013 and CUDA
%     Toolkit 6.5.
...
if isempty(opts.imageLibrary)
  switch arch
    case 'glnxa32', opts.imageLibrary = 'libjpeg' ;
    case 'maci32', opts.imageLibrary = 'quartz' ;
...
if opts.enableGpu
  ...
  switch arch
    case 'win32', opts.cudaLibDir = fullfile(opts.cudaRoot, 'lib', 'x32') ;
    case 'maci32', opts.cudaLibDir = fullfile(opts.cudaRoot, 'lib') ;
    case 'glnxa32', opts.cudaLibDir = fullfile(opts.cudaRoot, 'lib32') ;
...
if opts.enableCudnn
  ...
  switch arch
    case 'win32', opts.cudnnLibDir = fullfile(opts.cudnnRoot, 'lib', 'x32') ;
    case 'maci32', opts.cudnnLibDir = fullfile(opts.cudnnRoot, 'lib') ;
    case 'glnxa32', opts.cudnnLibDir = fullfile(opts.cudnnRoot, 'lib32') ;
    otherwise, error('Unsupported architecture ''%s''.', arch) ;
  end
end  
...
if strcmp(arch, 'maci32') && opts.enableGpu && cuver < 70000
  ...
  if  ~verLessThan('matlab', '8.5.0')
    ...
    flags.link{end+1} = 'LINKLIBS=$LINKLIBS -L"$MATLABROOT/bin/maci32" -lmx -lmex -lmat -lstdc++' ;
  end
end
if strcmp(arch, 'maci32') && opts.enableGpu
...
if status == 1
  ...
  cl_path = fullfile(cc.Location, 'VC','bin','x86_amd32');
...
if ~valid || cuver_ ~= cuver
  ...
  switch computer
    case 'PCWIN32', separator = ';' ;
    case {'GLNXA32', 'MACI32'}, separator = ':' ;
  end
